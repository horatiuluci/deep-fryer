...
def get_total_seconds(input_file):
    ...
    """
    deets = get_video_details(input_file)
    duration = deets[b'duration'].split(':')
...
def get_video_details(input_file):
    ...
    for l in lines:
        ...
        if l.startswith(b'Duration'):
            metadata[b'duration'] = re.search('Duration: (.*?),', l).group(0).split(':',1)[1].strip(' ,')
            metadata[b'bitrate'] = re.search("bitrate: (\d+ kb/s)", l).group(0).split(':')[1].strip()
        if l.startswith(b'Stream #0:0'):
            metadata[b'video'] = {}
            metadata[b'video']['codec'], metadata['video']['profile'] = \
                [e.strip(' ,()') for e in re.search('Video: (.*? \(.*?\)),? ', l).group(0).split(':')[1].split('(')]
            metadata[b'video']['resolution'] = re.search('([1-9]\d+x\d+)', l).group(1)
            metadata[b'video']['bitrate'] = re.search('(\d+ kb/s)', l).group(1)
            metadata[b'video']['fps'] = re.search('(\d+ fps)', l).group(1)
        if l.startswith(b'Stream #0:1'):
            metadata[b'audio'] = {}
            metadata[b'audio']['codec'] = re.search('Audio: (.*?) ', l).group(1)
            metadata[b'audio']['frequency'] = re.search(', (.*? Hz),', l).group(1)
            metadata[b'audio']['bitrate'] = re.search(', (\d+ kb/s)', l).group(1)
